-- ============================================================
-- Naive UTC-as-PT for shift logic
-- ============================================================
-- Stream timestamps are stored "naively" — the UTC hour/minute IS the
-- PT wall-clock time, with no actual offset conversion applied. This
-- way every viewer sees the same wall-clock regardless of their TZ,
-- and the calendar's grid (which renders UTC fields) lines up with
-- the (weekday, block) keys here.
--
-- So the RPCs extract DOW / hour / minute in UTC, treating the stored
-- value as the canonical wall-clock.
-- ============================================================
...
BEGIN
  ...
  -- Chain rule: same UTC-naive day adjacency
  v_block_size := (v_brand.block_size_minutes || ' minutes')::interval;

  SELECT COUNT(*) INTO v_other_count
  FROM public.streams s
  WHERE s.brand_id = p_brand_id
    AND (s.start_time AT TIME ZONE 'UTC')::date
